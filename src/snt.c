...
const char *sntGetVersion() { return SNT_STR_VERSION " (" SNT_ARCH ") -- " SNT_COMPILER_NAME; }
...
void sntReadArgument(int argc, char *const *argv, char *ip, unsigned int *port, SNTConnectionOption *option) {
					...
					if (i == g_bindconnection->tcpsock) {
						...
						if ((sntIsBenchEnable(con))) {
							...
							((pthread_t *)g_threadtable)[sntPoolGetIndex(g_connectionpool, con)] =
								sntBenchmarkCreateThread(con->option->bm_protocol_mode, con);
						}
					}
				}
...
int sntInitServer(unsigned int port, SNTConnectionOption *option) {
	...
	sntVerbosePrintf("Creating connection pool %d connections, size %d bytes.\n", option->listen,
...
int sntInitClient(unsigned int nparallcon) {
	...
	sntVerbosePrintf("Creating connection pool %d connections, size %d.\n", nparallcon,
					 sizeof(SNTConnection) * nparallcon);
